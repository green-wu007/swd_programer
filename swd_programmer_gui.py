...
BASE_WIDTH = 1480          # reference width for font scaling
MIN_SCALE  = 0.70
MAX_SCALE  = 2.00
...
def main(page: ft.Page):
    ...
    def _on_resize(e):
        w = page.window_width or BASE_WIDTH
    ...
    def _build_log_text() -> str:
        ...
        for item in reversed(state.logs):
            ...
            rows.append(
                f'#{item["idx"]} {item["ts"]} {item["ch"]}{mac_part} {item["msg"]} '
                f'| 本次S:{state.flash_ok_session} F:{state.flash_err_session} '
                f'累積S:{state.flash_ok_count} F:{state.flash_err_count}'
            )
        return "\n".join(rows) + ("\n" if rows else "")
    ...
    def _save_log(_=None):
        ...
        try:
            with path.open("a", encoding="utf-8") as f:
                f.write(_build_log_text())
            _add_log('SYS', 'ok', f'Log 已儲存（附加）: {path}')
        except Exception as ex:
            _add_log('SYS', 'err', f'Log 儲存失敗: {ex}')

    async def _save_log_as(_=None):
        ...
        try:
            ...
            with out.open("a", encoding="utf-8") as f:
                f.write(_build_log_text())
            _add_log('SYS', 'ok', f'Log 已另存（附加）: {out}')
        except Exception as ex:
    ...
    def _refresh_ports(_=None):
        ...
        if r_com_dd.current:
            r_com_dd.current.options = [
                ft.dropdown.Option(
                    key=p["device"],
                    content=ft.Text(p["label"], size=21, no_wrap=True),
                )
                for p in ports
            ]
    ...
    def _on_com_change(e):
        ...
        _add_log('SYS', 'muted', f'COM Port 切換為 {state.com_port}')
        page.update()
        ...
        return ft.Container(
            ...
            content=ft.Column(
                controls=[
                    ...
                    ft.Text(f['date'], size=state.fs(10),
                            color=ft.Colors.BLUE_200 if active
                            else ft.Colors.OUTLINE),
                ],
                spacing=3,
    ...
    def _on_swd_change(e):
        state.swd_freq = e.control.value
    ...
    swd_freq_dd = ft.Dropdown(
        options=[ft.dropdown.Option(content=ft.Text(f, size=21), key=f) for f in SWD_FREQS],
                            ...
                            ft.Row(
                                controls=[
                                    ...
                                    ft.Text(ref=r_ok_session, value='本次成功 0',
                                            size=22, color=ft.Colors.GREEN_300),
                                    ft.Text(ref=r_err_session, value='本次失敗 0',
                                            size=22, color=ft.Colors.RED_300),
